...
const STORAGE_KEY = "virtualStockMarket_v15";
const MARKET_OPEN = 8;
...
function generateDayHistory(company, dayDate, endTime, startClose, options = {}) {
  ...
  // 시초가만 갭 반영 후, 장중은 넥스트플레이와 동일하게 자유 등락
  let lastClose =
...
function createInitialState() {
  ...
  return { companies, createdAt: new Date().toISOString(), version: 15 };
}
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      if (parsed.companies?.A?.history?.[0]?.open != null && parsed.version === 15) {
        COMPANIES.forEach((c) => {
