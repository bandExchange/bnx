...
const STORAGE_KEY = "virtualStockMarket_v19";
const MARKET_OPEN = 8;
...
function generateHistory(company, endTime) {
  ...
  history = history.concat(
    generateDayHistory(company, end, end, lastClose, { applyOpenGap: true })
  );
...
function createInitialState() {
  ...
  return { companies, createdAt: new Date().toISOString(), version: 19 };
}
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      if (parsed.companies?.A?.history?.[0]?.open != null && parsed.version === 19) {
        COMPANIES.forEach((c) => {
