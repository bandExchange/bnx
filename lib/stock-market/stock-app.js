...
const STORAGE_KEY = "virtualStockMarket_v18";
const LEGACY_STORAGE_KEYS = ["virtualStockMarket_v19"];
/** 이 날짜(포함)부터 장 시작 시초가 갭 적용 */
const OPEN_GAP_START_DATE = "2026-07-21";
const MARKET_OPEN = 8;
...
function getOpenGapPrice(prevClose, openDayChangePct, limits) {
  return clampToDailyLimits(
    roundToPriceTick(prevClose * (1 + openDayChangePct)),
    limits
  );
}
...
function shouldApplyOpenGap(dayDate) {
  return getDayKey(dayDate) >= OPEN_GAP_START_DATE;
}

/** 새 거래일 시초가 (7/21 이전·당일 중간 캔들은 갭 없음) */
function getSessionOpenPrice(company, history, prevClose, sessionStart) {
  let openPrice = roundToPriceTick(prevClose);
  if (company.openDayChangePct == null || !shouldApplyOpenGap(sessionStart)) {
    return openPrice;
  }

  const lastHist = history[history.length - 1];
  const isNewSessionDay =
    !lastHist ||
    new Date(lastHist.timestamp).toDateString() !== new Date(sessionStart).toDateString();

  if (!isNewSessionDay) return openPrice;

  const limits = getDailyPriceLimits(prevClose);
  return getOpenGapPrice(prevClose, company.openDayChangePct, limits);
}
...
function generateDayHistory(company, dayDate, endTime, startClose, options = {}) {
  ...
  if (options.applyOpenGap && company.openDayChangePct != null) {
    lastClose = getOpenGapPrice(startClose, company.openDayChangePct, limits);
  }
  ...
  while (cursor.getTime() <= limit.getTime()) {
    ...
    if (mins >= MARKET_OPEN_MIN && mins <= LAST_CANDLE_MIN) {
      ...
      const candle = generateCandle(lastClose, vol, seed, limits);
...
function generateHistory(company, endTime) {
  ...
  history = history.concat(
    generateDayHistory(company, end, end, lastClose, {
      applyOpenGap: shouldApplyOpenGap(end),
    })
  );
...
function ensureLiveCandle(company) {
  ...
  const openPrice = getSessionOpenPrice(company, history, lastClose, start);
  cs.liveCandle = createLiveCandle(start, openPrice);
...
function finalizeLiveCandle(company) {
  ...
  const openPrice = getSessionOpenPrice(company, cs.history, lastClose, nextStart);
  cs.liveCandle = createLiveCandle(nextStart, openPrice);
  return true;
}
...
function createInitialState() {
  ...
  COMPANIES.forEach((c) => {
    ...
    const openPrice = getSessionOpenPrice(c, history, lastClose, liveStart);

    companies[c.id] = {
      history,
      lastCandle: endTime.toISOString(),
      liveCandle: createLiveCandle(liveStart, openPrice),
    };
  });

  return { companies, createdAt: new Date().toISOString(), version: 18 };
}
...
function loadState() {
  ...
  try {
    for (const key of keys) {
      ...
      if (parsed.companies?.A?.history?.[0]?.open != null && parsed.version >= 18) {
        COMPANIES.forEach((c) => {
