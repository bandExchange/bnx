...
const STORAGE_KEY = "virtualStockMarket_v13";
const MARKET_OPEN = 8;
...
function createInitialState() {
  ...
  return { companies, createdAt: new Date().toISOString(), version: 13 };
}
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      if (parsed.companies?.A?.history?.[0]?.open != null && parsed.version === 13) {
        COMPANIES.forEach((c) => {
