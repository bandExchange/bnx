...
const STORAGE_KEY = "virtualStockMarket_v17";
const MARKET_OPEN = 8;
...
/** 장중 진행률 0~1 (시가→종가) */
function getSessionProgress(atTime) {
  const mins = toDayMinutes(atTime);
  if (mins <= MARKET_OPEN_MIN) return 0;
  if (mins >= MARKET_CLOSE_MIN) return 1;
  return (mins - MARKET_OPEN_MIN) / (MARKET_CLOSE_MIN - MARKET_OPEN_MIN);
}

function getCloseTargetPrice(prevClose, closeDayChangePct, limits) {
  return clampToDailyLimits(
    roundToPriceTick(prevClose * (1 + closeDayChangePct)),
    limits
  );
}

/** 해당 시각의 경로 목표가 (마감 시 전일대비 목표로 선형 수렴) */
function getPathTargetPrice(prevClose, closeDayChangePct, atTime, limits) {
  const closeTarget = getCloseTargetPrice(prevClose, closeDayChangePct, limits);
  const t = getSessionProgress(atTime);
  return clampToDailyLimits(
    roundToPriceTick(prevClose + (closeTarget - prevClose) * t),
    limits
  );
}

/** 마감 목표 경로 주변에서 랜덤 등락하는 캔들 */
function generatePathCandle(openPrice, pathTarget, seed, limits, snapToTarget) {
  const open = clampToDailyLimits(openPrice, limits);
  const band = Math.max(
    PRICE_TICK * 4,
    roundToPriceTick(Math.abs(pathTarget - open) * 0.35 + pathTarget * 0.003)
  );
  const bandLimits = {
    lower: Math.max(limits.lower, roundToPriceTick(pathTarget - band)),
    upper: Math.min(limits.upper, roundToPriceTick(pathTarget + band)),
  };
  ...
  for (let i = 0; i < 4; i++) {
    const dir = seededRandom(seed + i * 19) >= 0.5 ? 1 : -1;
    const ticks = 1 + Math.floor(seededRandom(seed + i * 7) * 5);
    price = clampToDailyLimits(price + dir * ticks * PRICE_TICK, bandLimits);
    if (seededRandom(seed + i * 11) < 0.5) {
      price = clampToDailyLimits(
        price + (price < pathTarget ? 1 : -1) * ticks * PRICE_TICK,
        bandLimits
      );
    }
    high = Math.max(high, price);
  ...
  const close = snapToTarget
    ? clampToDailyLimits(pathTarget, limits)
    : clampToDailyLimits(price, bandLimits);
  ...
  return {
    open,
    high: clampToDailyLimits(high, limits),
    low: clampToDailyLimits(low, limits),
    close,
    volume,
    changeRate: open === 0 ? 0 : ((close - open) / open) * 100,
  };
...
function generateDayHistory(company, dayDate, endTime, startClose) {
  ...
  const useCloseTarget = company.closeDayChangePct != null;
  ...
  while (cursor.getTime() <= limit.getTime()) {
    ...
    if (mins >= MARKET_OPEN_MIN && mins <= LAST_CANDLE_MIN) {
      ...
      const candleEnd = new Date(cursor.getTime() + CANDLE_MS);
      const isLastCandle = mins >= LAST_CANDLE_MIN;
      let candle;

      if (useCloseTarget) {
        const pathTarget = getPathTargetPrice(
          startClose,
          company.closeDayChangePct,
          isLastCandle ? setDayMinutes(cursor, MARKET_CLOSE_MIN) : candleEnd,
          limits
        );
        candle = generatePathCandle(
          lastClose,
          pathTarget,
          seed,
          limits,
        ...
        candle = generateCandle(lastClose, vol, seed, limits);
      }
...
function generateHistory(company, endTime) {
  ...
  history = history.concat(generateDayHistory(company, end, end, lastClose));
...
function getHourlyPriceMove(company, now = new Date()) {
  if (company.closeDayChangePct != null) {
    const cs = state.companies[company.id];
    const live = cs.liveCandle;
    ...
    const band = Math.max(PRICE_TICK * 3, roundToPriceTick(pathTarget * 0.004));
    const lower = Math.max(limits.lower, pathTarget - band);
    const upper = Math.min(limits.upper, pathTarget + band);

    // 마감에 가까울수록 목표로 강하게 수렴
    const progress = getSessionProgress(now);
    const pull = progress > 0.85 ? 0.85 : 0.35;
    const toward = live.close < pathTarget ? 1 : live.close > pathTarget ? -1 : 0;
    const randomDir = Math.random() < 0.5 ? -1 : 1;
    const ticks = 1 + Math.floor(Math.random() * 3);
    let dir = Math.random() < pull ? toward || randomDir : randomDir;
    let next = live.close + dir * ticks * PRICE_TICK;

    if (next < lower) next = lower + PRICE_TICK;
    if (next > upper) next = upper - PRICE_TICK;

    // 장 마감 직전: 목표가에 고정
    if (progress >= 0.98) {
      next = getCloseTargetPrice(prevClose, company.closeDayChangePct, limits);
    }

    return next - live.close;
  }
...
function applyTimeTick(now = new Date()) {
  ...
  COMPANIES.forEach((company) => {
    ...
    const limits = getDailyLimitsForHistory(
      cs.history,
      now,
      cs.history.at(-1)?.close ?? company.basePrice
    );
    const move = getHourlyPriceMove(company, now);
    const nextPrice = clampToDailyLimits(live.close + move, limits);
...
function createInitialState() {
  ...
  return { companies, createdAt: new Date().toISOString(), version: 17 };
}
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      if (parsed.companies?.A?.history?.[0]?.open != null && parsed.version === 17) {
        COMPANIES.forEach((c) => {
